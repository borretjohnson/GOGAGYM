/**
 * GOGAGYM — Premium Fitness App v3
 * Исправлена загрузка на телефоне
 */
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🩸 GOGAGYM: загрузка...');
    try {
        initTelegram();
        initParticles();
        loadData();
        buildOnboarding();
        if (appData.user.onboardingComplete) { 
            showApp(); 
        } else {
            showOnboarding();
        }
        ...
        document.getElementById('onboarding').innerHTML = `
            <div class="onboarding-content">
                <div class="onboarding-logo">
                    <span class="logo-big">GOGA<span class="text-red">GYM</span></span>
                </div>
                <p style="color: #ff4444; margin: 20px 0;">Ошибка загрузки</p>
                <button class="btn-onboard" onclick="location.reload()">🔄 Обновить</button>
            </div>
        `;
    }
});
...
function initTelegram() {
    try {
        ...
        console.warn('⚠️ Telegram WebApp не доступен:', e);
    }
}
...
function buildOnboarding() {
    const html = `
        ...
        <h1 class="onboarding-title">ТВОЙ ПУТЬ ВОИНА</h1>
        <p class="onboarding-desc">Персональный тренер в твоём кармане</p>
        <div class="onboarding-form">
            <div class="form-group">
                <label>🎯 ТВОЯ ЦЕЛЬ</label>
                <select id="onboard-goal">
                    <option value="burn">🔥 Сжигание жира</option>
            ...
            <div class="form-group">
                <label>📍 ГДЕ ТРЕНИРУЕШЬСЯ</label>
                <div class="place-selector">
                    <button class="place-btn" data-place="gym" onclick="selectPlace('gym')">
                        <span class="place-icon">🏋️</span><span>В ЗАЛЕ</span>
                    </button>
                    <button class="place-btn" data-place="home" onclick="selectPlace('home')">
                        <span class="place-icon">🏠</span><span>ДОМА</span>
                    </button>
                </div>
            </div>
            <div class="form-group">
                <label>⚖️ ПАРАМЕТРЫ</label>
                <div class="params-row">
                    <input type="number" id="onboard-weight" placeholder="Вес (кг)">
                    <input type="number" id="onboard-height" placeholder="Рост (см)">
                    <input type="number" id="onboard-age" placeholder="Возраст">
                </div>
            </div>
            <button class="btn-onboard" onclick="completeOnboarding()">
                <span>🚀 НАЧАТЬ ПУТЬ</span>
            </button>
        </div>
    `;
    document.getElementById('onboarding').innerHTML = `<div class="onboarding-content">${html}</div>`;
}
...
function completeExercise() {
    ...
    if (Object.keys(weights).length > 0) {
        ...
        appData.weightsHistory.unshift({ date: new Date().toISOString(), exercise: exercise?.name || 'Упражнение', weights: weights });
    }
